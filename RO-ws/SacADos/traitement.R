# Author : SV
# date 2015/10/12

############################
# recherche al̩atoire

# lit les donn̩es des recherches al̩atoires
df.rs <- read.table("sa.csv", header = TRUE, sep = " ")

# v̩rification du fichier: affiche les premi̬res lignes
head(df.rs)

# histogramme pour un nombre d'̩valuation 10000 : nota, la distribution n'est pas gaussienne
hist(df.rs[df.rs$nbeval == 100000,]$fitness)

# calcul de la moyenne
mean(df.rs[df.rs$nbeval == 100000,]$fitness)

# r̩sum̩ des principales statistiques
summary(df.rs[df.rs$nbeval == 100000,]$fitness)

# nuage de points des fitness obtenues en fonction du nombre d'̩valuation
plot(fitness ~ nbeval, data = df.rs)

# chargement de la librairie doBy
library(doBy)

# calcul des moyennes de fitness en fonction du nombre d'̩valuation
df.rs.mean <- summaryBy(fitness ~ nbeval, data = df.rs)

# plot correspondant
plot(fitness.mean ~ nbeval, data = df.rs.mean)

# grosso modo, on a l'impression que les donn̩es suivent une loi logarithmique
# d'ailleur, on peut tracer avec le log:
plot(fitness.mean ~ log(nbeval), data = df.rs.mean)

# et m̻me fitter un mod̬le lin̩aire
model <- lm(fitness.mean ~ log(nbeval), data = df.rs.mean)

# trace la droite de r̩gression
abline(model)
# donne le descriptif du mod̬le : noter un tr̬s bon coefficient R^2
summary(model)

## Remarque : sans la librairie doBy 

# split des donn̩es selon les valeur en ligne (obtient alors une liste)
l <- split(df.rs$fitness, factor(df.rs$nbeval))

# boite a moustache (boxplot) de la fitness en fonction du nombre d'̩valuation (attention echelle non respect̩e en abscisse)
boxplot(l)

# calcul des moyennes (nota: R est un langage fonctionnel)
l.mean <- lapply(l, FUN = mean)

# et le plot fitness vs. moyenne
plot(names(l.mean), unlist(l.mean))


############################
# marche al̩atoire

# lit les donn̩es de la marche al̩atoire
df.rw <- read.table("sa.csv", header = TRUE, sep = " ")

# v̩rification du fichier: affiche les premi̬res lignes
head(df.rw)

# histogramme des fitness : nota, la distribution n'est pas gaussienne
hist(df.rw$fitness)

# fitness en fonction du nombre d'it̩ration. Nota : les variations sont "al̩atoires" mais born̩es.
plot(df.rw$fitness)

# calcul de la fonction d'autocorrelation : beaucoup de correlation entre les fitness des solutions voisines
cors <- acf(df.rw$fitness)

# le premier coefficient d'autocorrelation est tr̬s proche de 1:
print(cors[1])

